  ...
  connection.connect(function connectionAttempted (err) {
    if (err) {
      ...
      async.parallel({
        ...
        startingDollar2014: function (callback) {
          /*jshint multistr: true */
          var query = 'SELECT sum(2014dollar) AS "sumdollar" \
                  FROM gauntlet.amounts \
                  WHERE year(yesdate) < 2014 ;';
          connection.query(query, function queryComplete (err, result) {
            if(err) {
      ...
      function (err, results) {
        ...
        console.log(results.startingDollar2014[0].sumdollar);

        // build up a JSON object with the 12 months
        var output = [];
        var runningTotalDollar = results.startingDollar2014[0].sumdollar; // money coming in in 2014 from prior years
        ...
        for (var i = 1; i <= 12; i++) {
          ...
          for (var k = results.contributors.length - 1; k >= 0; k--) {
            if (results.contributors[k].month === i && results.contributors[k].year === 2014) {
